    ...
    export default {
        ...
        methods: {
            ...
            async getLoginjoinslist(startDate, endDate) {
                ...
                try {
                    self.loading = true;
                    const res = await logapi.getLoginList(appID, roomID, startDate, endDate);
                    self.loading = false;
                    if (res.status === 'OK') {
                ...
                try {
                    self.loading = true;
                    const res = await logapi.getJoinList(appID, roomID, startDate, endDate);
                    self.loading = false;
                    if (res.status === 'OK') {
            ...
            async getLoginJoin(startDate, endDate) {
                ...
                try {
                    ...
                    const res = await logapi.getLoginjoinslist(appID, roomID, startDate, endDate, from, self.pageSize);
                    if (res.status === 'OK') {
