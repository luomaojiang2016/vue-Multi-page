    ...
    export default {
        ...
        methods: {
            ...
            async getPublishStreamCount(startDate, endDate) {
                ...
                try {
                    const res = await logapi.getPublishStreamCount(appServerID, accountID, appID, roomID, startDate, endDate);
                    if (res.status === "OK") {
            ...
            async getSubscribeStreamCount(startDate, endDate) {
                ...
                const roomID = self.roomID === '' ? 0 : self.roomID;
                try {
                    const res = await logapi.getSubscribeStreamCount(appServerID, accountID, appID, roomID, startDate, endDate);
                    if (res.status === "OK") {
            ...
            async getPublishStreamFlow(startDate, endDate) {
                ...
                const roomID = self.roomID === '' ? 0 : self.roomID;
                try {
                    const res = await logapi.getPublishStreamFlow(appServerID, accountID, appID, roomID, startDate, endDate);
            ...
            async getSubscribeStreamFlow(startDate, endDate) {
                ...
                const roomID = self.roomID === '' ? 0 : self.roomID;
                try {
                    const res = await logapi.getSubscribeStreamFlow(appServerID, accountID, appID, roomID, startDate, endDate);
                    if (res.status === "OK") {
            ...
            async getPublishStreamSpeed(startDate, endDate) {
                ...
                const roomID = self.roomID === '' ? 0 : self.roomID;
                try {
                    const res = await logapi.getPublishStreamSpeed(appServerID, accountID, appID, roomID, startDate, endDate);
            ...
            async getSubscribeStreamSpeed(startDate, endDate) {
                ...
                const roomID = self.roomID === '' ? 0 : self.roomID;
                try {
                    const res = await logapi.getSubscribeStreamSpeed(appServerID, accountID, appID, roomID, startDate, endDate);
                    if (res.status === "OK") {
